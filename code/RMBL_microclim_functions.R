...
micro_csv <- function(csv_name,data_column=4) {
  ...
  is.mx <- any(grepl("#",header)) && any(grepl("*C,",header))
  is.mx2 <- any(grepl("#,Date-Time",header))
  is.tms <- grepl("0;",header[1]) | grepl("1;",header[1])
  ...
  else if (is.mx==TRUE | is.mx2==TRUE) {
    # do this if the sensor is an Onset MX logger
    sample <- header[c((length(header)-10):length(header))]
    sample_split <- unlist(strsplit(sample,split=","))
    is.comma.del <- length(sample)!=length(sample_split)
    if(is.comma.del) {
      file <- read.table(csv_name, header = F, sep = ",",skip=1,fill=TRUE)  # read in the raw data .csv file (as comma delimited) as a dataframe
    }
    else{
      file <- read.table(csv_name,header = F, sep="\t",skip=1,fill=TRUE)    # read in the raw data .csv file (as tab delimited) as a dataframe
    }
    if(ncol(file)<4){
      file[,4] <- NA
    }
    file <- file[,2:4]  # remove all columns except 2 and 3 (date/time and temperature)
  }  # end MX-specific procedure
  else if(is.tms==TRUE){
    if(grepl(",",header[2])){
      file <- read.table(csv_name, header = F, sep=";",skip=0,fill=TRUE,dec=",")
    }else{
      file <- read.table(csv_name, header = F, sep=";",skip=0,fill=TRUE)
    }
    file <- file[,c(2,data_column)]
    if(is.na(strptime(as.character(file[,1])[1],format="%Y.%m.%d %H:%M",tz="GMT"))){
  ...
  if(any(grepl("GMT-08:00", header,fixed=T)) | any(grepl("PDT", header,fixed=T)) | any(grepl("PST", header,fixed=T))){
    tz <- "US/Pacific-New"
  }
  else if(any(grepl("MDT", header,fixed=T)) | any(grepl("GMT-07:00", header,fixed=T))){
    tz <- "US/Mountain"
  }
  ...
  dashed <- grepl("-",datestring)
  valid.year <- any(sapply(valid_years,FUN=yeartest))
  year.first <- as.numeric(substr(datestring,1,4)) %in% valid_years
  ...
  }else if(am.pm==FALSE && valid.year==TRUE && dashed==FALSE) {
    dateTime <- strptime(file$DateTime, "%m/%d/%Y %H:%M")
  }else if(am.pm==FALSE && valid.year==TRUE && dashed==TRUE && year.first==FALSE) {
    dateTime <- strptime(file$DateTime, "%m-%d-%Y %H:%M")
  }else if(am.pm==FALSE && valid.year==TRUE && dashed==TRUE && year.first==TRUE) {
    dateTime <- strptime(file$DateTime, "%Y-%m-%d %H:%M")
...
merge_micro_csv <- function(input_path=NULL,
  ...
  stopifnot(is.logical(merge_by_site) & length(merge_by_site)==1)
  ...
  stopifnot(sensor_metadata_site_column %in% colnames(sensors))
  ...
  ## Sets up a sequence of hours for summarization
  hour_seq <- seq(hour_begin,hour_end,by="hour")
  na_seq <- rep(NA,length(hour_seq))
  empty_xts <- xts(na_seq,order.by=hour_seq,tzone=tzone)
  
  ## Finds sites for matched sensor names.
  matching_df <- data.frame(sensor=metadata_files_matched[,file_metadata_join_column],
                            site=metadata_files_matched[,sensor_metadata_site_column])
  matching_unique <- matching_df[!duplicated(matching_df),]
  
  matched_names <- as.character(matching_unique$sensor)
  unique_names <- unique(matched_names)
  matched_sites <- as.character(matching_unique$site)
  unique_sites <- unique(matched_sites)
  
  ##Sets up empty df for metadata and file counters.
  if(merge_by_site==FALSE){
    out_meta <- data.frame(merged_sitename = unique_names,
                           input_files = rep(NA,length(unique_names)),
                           n_input_files = rep(NA,length(unique_names)),
                           n_meas = rep(NA,length(unique_names)),
                           output_filename = rep(NA,length(unique_names)),
                           start_datetime = rep(NA,length(unique_names)),
                           end_datetime = rep(NA,length(unique_names)))
    merge_length <- length(unique_names)
  }else{
    out_meta <- data.frame(merged_sitename = unique_sites,
                           input_files = rep(NA,length(unique_sites)),
                           n_input_files = rep(NA,length(unique_sites)),
                           n_meas = rep(NA,length(unique_sites)),
                           output_filename = rep(NA,length(unique_sites)),
                           start_datetime = rep(NA,length(unique_sites)),
                           end_datetime = rep(NA,length(unique_sites)))
    merge_length <- length(unique_sites)
  }
  ...
  for(i in 1:merge_length){
    ...
    if(merge_by_site==FALSE){
      sensor_files <- metadata_files_matched[metadata_files_matched[,file_metadata_join_column]==unique_names[i],"out_filename"]
      print(paste("Merging",length(sensor_files),"time-series for sensor ", unique_names[i],
                  "(",i,"of",length(unique_names),")"))
    }else{
      sensor_files <- metadata_files_matched[metadata_files_matched[,sensor_metadata_site_column]==unique_sites[i],"out_filename"]
      print(paste("Merging",length(sensor_files),"time-series for site ", unique_sites[i],
                  "(",i,"of",length(unique_sites),")"))
    }
    
    series <- read.csv(paste(input_path,"/",sensor_files[1],sep=""),header=TRUE)
    datestring <- paste(paste(series$YEAR,series$MONTH,series$DAY,sep="-"), series$TIME,sep=" ")
    series$datetime <- as.POSIXct(datestring,tz=as.character(series$TZ[1]),format=c("%Y-%m-%d %H:%M:%S"))
    ...
    if(length(sensor_files)>1){
      for(k in 2:length(sensor_files)){
        ...
        datestring_new <- paste(paste(series_new$YEAR,series_new$MONTH,series_new$DAY,sep="-"), series_new$TIME,sep=" ")
        series_new$datetime <- as.POSIXct(datestring_new,tz=as.character(series$TZ[1]),format=c("%Y-%m-%d %H:%M:%S"))
    ...
    if(merge_by_site==FALSE){
      fig_name <- paste(figure_path,"/",unique_names[i],"_",
      ...
      if(file.exists(fig_name) & overwrite==FALSE){
        ...
        plot(df_out$TEMP~df_out$DATETIME,main=unique_names[i],ylim=c(-20,50),pch=".",
             ylab="Temp (C)",xlab="Date")
        abline(0,0,lty=2)
        dev.off()
      }
    }else{
      fig_name <- paste(figure_path,"/",unique_sites[i],"_",
                        gsub("-","_",as.character(as.Date(start_datetime))),"_",
                        gsub("-","_",as.character(as.Date(end_datetime))),
                        "_hourly.pdf",sep="")
      if(file.exists(fig_name) & overwrite==FALSE){
        ...
        pdf(fig_name,width=20,height=5)
        plot(df_out$TEMP~df_out$DATETIME,main=unique_sites[i],ylim=c(-20,50),pch=".",
             ylab="Temp (C)",xlab="Date")
        abline(0,0,lty=2)
        dev.off()
      }
    ...
    ##Prepares output file name.
    if(merge_by_site==FALSE){
      outfile <- paste(output_path,"/",unique_names[i],"_",
                       gsub("-","_",as.character(as.Date(start_datetime))),"_",
                       gsub("-","_",as.character(as.Date(end_datetime))),
                       "_hourly.csv",sep="")
    }else{
      outfile <- paste(output_path,"/",unique_sites[i],"_",
                       gsub("-","_",as.character(as.Date(start_datetime))),"_",
                       gsub("-","_",as.character(as.Date(end_datetime))),
                       "_hourly.csv",sep="")
    }
...
clean_air_temps <- function(input_path = NULL,
  ...
  for (i in 1:length(airtemp_files)){
    ...
    daily_max <- apply.daily(tempts,FUN=max,na.rm=TRUE)
    ...
    if((sum(is.na(tempts)) / length(tempts)) > 0.40){
...
summarise_airtemp_daily <- function(input_path,
  ...
  for(i in 1:length(raw_files)){
    ...
    if(file.exists(out_file) & overwrite==FALSE){
      ...
      ts_min_daily <- apply.daily(ts,FUN=min,na.rm=FALSE)
      ts_max_daily <- apply.daily(ts,FUN=max,na.rm=FALSE)
      ts_avg_daily <- apply.daily(ts,FUN=mean,na.rm=FALSE)
      df <- data.frame(DATE=as.Date(index(ts_avg_daily)),
                       MIN_TEMP=ts_min_daily,
                       MAX_TEMP=ts_max_daily,
                       AVG_TEMP=ts_avg_daily)
      
      ##Trims first and last days if specified.
      if(trim==TRUE){
        date_min <- which.min(df$DATE)
        date_max <- which.max(df$DATE)
        df <- df[-c(date_min,date_max),]
      }
...
qc_soil_temps <- function(input_path = NULL,
                            input_metadata_filename = NULL,
                            output_path = NULL,
                            output_metadata_filename = NULL,
                            figure_path = NULL,
  ...
  setwd(output_path)
  if(!dir.exists("./qc_flagged")){
    dir.create("./qc_flagged")
  }
  ...
  for (i in 1:length(soiltemp_files)){
    ...
    daily_range <- apply.daily(tempts$TEMP,FUN=range_fun)
    daily_mean <- apply.daily(tempts$TEMP,FUN=mean,na.rm=TRUE)
    ...
    ## Checks for and corrects Celcius / Fahrenheit problems
    if(max(tempts$TEMP) > cf_test_params[1] & max(tempts$TEMP) < cf_test_params[2] &
       min(tempts$TEMP) > cf_test_params[3] & mean(tempts$TEMP) > cf_test_params[2]){
      print(paste("Found possible C / F problem, correcting..."))
      tempts$TEMP <- (tempts$TEMP - 32) * (5/9)
    }
    
    ## Checks for and eliminates values outside of the physical range 
    ## and temperature spikes that could be caused by direct sun.
    tempts$FLAG[tempts$TEMP < min_temp_thresh] <- "R"
    tempts$FLAG[tempts$TEMP > max_temp_thresh] <- "R"
    tlagged <- lag.xts(tempts$TEMP,k=1)
    delta <- as.numeric(tempts$TEMP)-as.numeric(tlagged$TEMP)
    tempts$FLAG[delta > temp_spike_thresh] <- "S"
    
    ##Lab days are periods of near-constant temps at lab temp +/- one day for deployment.
    range_fun <- function(x){range(as.numeric(x),na.rm=TRUE)[2] - range(as.numeric(x),na.rm=TRUE)[1]}
    daily_range <- apply.daily(tempts$TEMP,FUN=range_fun)
    daily_mean <- apply.daily(tempts$TEMP,FUN=mean,na.rm=TRUE)
    lab_days <- as.Date(index(daily_range)[daily_range < 2 & daily_mean > 20 & daily_mean < 26])
    lab_days <- c(lab_days,(lab_days-1),(lab_days+1))
    
    ##Air days are periods with high temperature range, probably because the sensor was pulled out of the soil.
    air_days <- as.Date(index(daily_range)[daily_range > temp_spike_thresh])
    
    
    ##Sets lab measurements to zero.
    if(length(lab_days) > 0){
      print(paste("Possible lab days found, flagging..."))
      day_index <- as.Date(index(tempts))
      lab_index <- which(day_index %in% lab_days)
      tempts$FLAG[lab_index] <- "L"
    }
    
    if(length(air_days) > 0){
      print(paste("Possible air days found, flagging..."))
      day_index <- as.Date(index(tempts))
      air_index <- which(day_index %in% air_days)
      tempts$FLAG[air_index] <- "A"
    }
    
    ##Flags time-series with lots of problems
    if((sum(!is.na(tempts$FLAG)) / length(tempts)) > 0.20){
      flag_missing_values <- TRUE
    }else{
      flag_missing_values <- FALSE
    }
    ...
    outts <- data.frame(DATE=index(tempts),
                        YEAR=as.numeric(format(index(tempts),format="%Y")),
                        MONTH=as.numeric(format(index(tempts),format="%m")),
                        DAY=as.numeric(format(index(tempts),format="%d")),
                        TIME=format(index(tempts),format="%H:%M:%S"),
                        TZ=out_tz,
                        TEMP=tempts$TEMP,
                        FLAG=tempts$FLAG)
    ...
    # Save figure as pdf
    setwd(figure_path)
    plotname <- paste("qc_flagged/",strsplit(soiltemp_files[i], ".csv")[[1]],sep="")
    figpath <- paste(figure_path,"/",plotname,".pdf", sep = "")
    pdf(file = figpath, width = 10, height = 7)
    
    # Computes month boundaries for plotting.
    month_plot <- seq(min(outts$DATE),max(outts$DATE),by="month")
    month_min <- rep(-10,length(months))
    month_max <- rep(35,length(months))
    
    # Left Y axis
    par(mar = c(4, 6, 3, 6))
    plot(outts$TEMP[is.na(outts$FLAG)] ~ outts$DATE[is.na(outts$FLAG)], main = plotname, xlab='Date', pch=".",
         ylab=expression(paste("Temperature", degree, "C")),col='black',
         axes = T,ylim=c(-10,35))
    points(outts$TEMP[outts$FLAG=="R"] ~ outts$DATE[outts$FLAG=="R"], col='orange', pch=".")
    points(outts$TEMP[outts$FLAG=="S"] ~ outts$DATE[outts$FLAG=="S"], col='red', pch=".")
    points(outts$TEMP[outts$FLAG=="A"] ~ outts$DATE[outts$FLAG=="A"], col='blue', pch=".")
    points(outts$TEMP[outts$FLAG=="L"] ~ outts$DATE[outts$FLAG=="L"], col='magenta', pch=".")
    points(outts$TEMP[outts$FLAG=="O"] ~ outts$DATE[outts$FLAG=="O"], col='green', pch=".")
    legend("topright", legend=c("Unflagged","Range","Spike","Air","Lab","Other"), 
           col=c("black","orange","red","blue","magenta","green"), pch=16)
    dev.off()

    ## Writes output, dropping "date" column because of auto-formatting problems in Excel.
    setwd(output_path)
    outts <- outts[,-1]
    write.csv(outts,paste("./qc_flagged/", soiltemp_files[i],sep=""), row.names=FALSE)
...
####Function to clean qc-flagged time-series.####

##USAGE: Cleans qc-flagged time-series by replacing flagged values with NA.
clean_micro_csv <- function(input_path=NULL,
                            output_path=NULL,
                            figure_path=NULL,
  ...
  stopifnot(dir.exists(figure_path))
  stopifnot(is.logical(overwrite) & length(overwrite)==1)
  stopifnot(length(list.files(input_path,full.names=TRUE)) != 0)
  ...
  ##Creates folder for figure output
  setwd(figure_path)
  if(!dir.exists("./qc_cleaned")){
    dir.create("./qc_cleaned")
  }
  
  ##Loops through each file and cleans qc flags.
  for(i in 1:length(input_files)){
    
    setwd(input_path)
    
    if(overwrite==FALSE){
      file_exists <- file.exists(paste(output_path,input_files[i],sep=""))
      if(file_exists){
        print(paste("File", input_files[i], "already exists in output directory, skipping..."))
        next
      }
    }
    
    print(paste("Cleaning file", input_files[i], "(file", i, "of", length(input_files), ")..."))
    file <- read.csv(input_files[i],header=TRUE)
    ...
    ##Remove flags
    file$TEMP[file$FLAG %in% remove_flagged] <- NA
    
    ##Converts date to POSIXct
    datestring <- paste(paste(file$YEAR,file$MONTH,file$DAY,sep="-"), file$TIME, sep=" ")
    dates <- as.POSIXct(datestring,format="%Y-%m-%d %H:%M:%S",tz=file$TZ[1])
    
    ##Creates figure with cleaned output.
    setwd(figure_path)
    plotname <- paste("qc_cleaned/",strsplit(input_files[i], ".csv")[[1]],sep="")
    figpath <- paste(figure_path,"/",plotname,".pdf", sep = "")
    pdf(file = figpath, width = 10, height = 7)
    
    # Left Y axis
    par(mar = c(4, 6, 3, 6))
    plot(file$TEMP ~ dates, main = plotname, xlab='Date', pch=".",
         ylab=expression(paste("Temperature", degree, "C")),col='black',
         axes = T,ylim=c(-10,35))
    dev.off()
    ...
    ##Writes clean output.
    setwd(output_path)
    write.csv(file,paste("./", input_files[i],sep=""), row.names=FALSE)
  }
}
...
compile_soiltemp_daily <- function(input_path,
  ...
  max_ts_all <- merge(day_ts,first_max_ts,join="left")
  max_ts_all <- max_ts_all[,-1]
  if(strsplit(daily_names[1],split="_")[[1]][1]=="data"){
    sitenames[1] <- paste(strsplit(daily_names[1],split="_")[[1]][1:2],collapse="_")
  }else{
    sitenames[1] <- strsplit(daily_names[1],split="_")[[1]][1]
  }
  ...
  for (i in 2:length(daily_files)){
    ...
    if(strsplit(daily_names[1],split="_")[[1]][1]=="data"){
      sitenames[i] <- paste(strsplit(daily_names[i],split="_")[[1]][1:2],collapse="_")
    }else{
      sitenames[i] <- strsplit(daily_names[i],split="_")[[1]][1]
    }
